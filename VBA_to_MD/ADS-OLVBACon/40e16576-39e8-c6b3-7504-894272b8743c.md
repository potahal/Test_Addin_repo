

---
ms.Toctitle:DistListItem.TaskSubject プロパティ (Outlook)(機械翻訳)
title:DistListItem.TaskSubject プロパティ (Outlook)(機械翻訳)
ms.ContentId:40e16576-39e8-c6b3-7504-894272b8743c
---
# DistListItem.TaskSubject プロパティ (Outlook)(機械翻訳)




返すまたは、**配布リスト**オブジェクトのタスクの件名を表す**文字列**値を設定します。読み取り/書き込み。

## 構文

            UNRESOLVED_TOKEN_VAL(offexpression).**TaskSubject**




            UNRESOLVED_TOKEN_VAL(offexpression)
            **DistListItem** オブジェクトを返すオブジェクト式を指定します。



## 注釈
**IsMarkedAsTask**プロパティが**False**に設定されている場合、このプロパティは**Subject**プロパティの値を返します。



## Related Topics

[配布リスト オブジェクト](027c3986-abff-d9b1-ecc2-26d60805e952.md)

[配布リスト オブジェクトのメンバー](3ba4af84-ce84-61d9-1bc9-fab41bf6f125.md)




